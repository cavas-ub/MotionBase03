...
MotionBase::MotionBase (char *h)
{
  SetHost(h);
}

void MotionBase::SetHost(char *h)
{
  strcpy_s(IPAddress,h);
}


char* MotionBase::GetHost()
{
  return IPAddress;
}

void MotionBase::Update()
{
  UpdateFlag=true;
}
...
int MotionBase::Initialize()
{
 ...
 // rd test 
  ...
  //fpg = fopen("respstates.txt", "w+");
  //fpgg = fopen("sendstates.txt", "w+");
  
  // set the running flag up so the read and write threads continue
  Running=true;

  // Reset Heave to 0 for repeat runs.
  goal.c = 0.;
  
  // Fire off the read and write process, or processes.
  if(rev_comm == 0)
  {
	/*pthread_create(&write,NULL,start_W, new MotionBase);
    pthread_create(&read,NULL,start_R, new MotionBase);*/

	
	//auto wt = std::bind(&WriteMotionBase, this);
	writeThread = std::thread(start_R, new MotionBase);

	//auto rt = std::bind(&ReadMotionBase, this);
	readThread = std::thread(start_R, new MotionBase);

  }
  else
  {
	  //ReadWriteMotionBase
	  //pthread_create(&readwrite,NULL,start_RW, new MotionBase);
	  //auto rwt = std::bind(&ReadWriteMotionBase, this);
	  readwriteThread = std::thread(start_RW, new MotionBase);
  }
  ...
  if(rev_comm == 0)
  {
	  if(motion_mode == 0)
		  ...
		  SetMotionCommand(swapbits(MDA_MODE));
  }
...
void MotionBase::SetRoll(float f)
{
  float range;
  if(f>1.0) f=1.0;
  else
...
void MotionBase::SetPitch(float f)
{
  float range;
  if(f>1.0) f=1.0;
  else
...
void MotionBase::SetYaw(float f)
{
  float range;
  if(f>1.0) f=1.0;
  else
...
void MotionBase::SetHeave(float f)
{
  float range;
  if(f>1.0) f=1.0;
  else
...
void MotionBase::SetSurge(float f)
{
  float range;
  if(f>1.0) f=1.0;
  else
...
void MotionBase::SetSway(float f)
{
  float range;
  if(f>1.0) f=1.0;
  else
...
void MotionBase::ReadMotionBase(void)
{
  int idum=1;
  int result=-1, res2=-1;
  fd_set readset;
...
void MotionBase::ReadWriteMotionBase(void)
{
  int idum=1;
  int result=-1, res2=-1;
  fd_set readset;
...
void MotionBase::WriteMotionBase(void)
{
  CDuration dur;
